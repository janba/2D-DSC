...
//    }
//    glEnd();
    std::vector<std::vector<vec2> > subdomain_corners = domain.get_subdomain_corners();
    glColor3d(static_cast<double>(color[0]), static_cast<double>(color[1]), static_cast<double>(color[2]));
    vec2 p0, p1, p2;
    vec3 cor;
    int j = 0, i;
    glBegin(GL_TRIANGLES);
    for (auto c = 0; c<subdomain_corners.size(); c++)
    {
        std::vector<vec2> corners = subdomain_corners[c];
        while (corners.size() > 2)
        {
            i = (j+1)%corners.size();
            p0 = corners[j%corners.size()];
...
void Painter::draw_arrows(const DeformableSimplicialComplex& dsc, const HMesh::VertexAttributeVector<vec2> &arrows, vec3 color, bool origin_movable)
{
    glColor3d(static_cast<double>(color[0]), static_cast<double>(color[1]), static_cast<double>(color[2]));
    glLineWidth(std::max(std::floor(LINE_WIDTH*dsc.get_avg_edge_length()), 1.));
    vec3 arrow, a_hat, p;
...
void Painter::draw_edge_arrows(const DeformableSimplicialComplex& dsc, const HMesh::HalfEdgeAttributeVector<vec2> &arrows, vec3 color)
{
    glColor3d(static_cast<double>(color[0]), static_cast<double>(color[1]), static_cast<double>(color[2]));
    glLineWidth(std::max(std::floor(LINE_WIDTH*dsc.get_avg_edge_length()), 1.));
    vec3 arrow, a_hat, p;
...
void Painter::draw_destination_faces(const DeformableSimplicialComplex& dsc, const HMesh::FaceAttributeVector<vec3> &colors)
{
    glBegin(GL_TRIANGLES);
	for(auto fi = dsc.faces_begin(); fi != dsc.faces_end(); ++fi)
    {
...
void Painter::draw_destination_faces(const DeformableSimplicialComplex& dsc, const HMesh::FaceAttributeVector<double> &values)
{
    glBegin(GL_TRIANGLES);
	for(auto fi = dsc.faces_begin(); fi != dsc.faces_end(); ++fi)
    {
...
void Painter::draw_faces(const DeformableSimplicialComplex& dsc, const HMesh::VertexAttributeVector<double> &values)
{
    glBegin(GL_TRIANGLES);
	for(auto fi = dsc.faces_begin(); fi != dsc.faces_end(); ++fi)
    {
...
void Painter::draw_destination_faces(const DeformableSimplicialComplex& dsc, const HMesh::VertexAttributeVector<double> &values)
{
    glBegin(GL_TRIANGLES);
	for(auto fi = dsc.faces_begin(); fi != dsc.faces_end(); ++fi)
    {
